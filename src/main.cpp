...
#if defined(NDEBUG)
# error "lifecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x22a2b37f23647490b6517d0db76cfe80cbbfdb68fc6adc4d1685c185f17406b4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // lifecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "lifecoin Signed Message:\n";
...
{
    // lifecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // lifecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // lifecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1470394507, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "fgdfgf";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("043819b6f6c65eb4d989359af7b2d6af59b6541e8ea60b421a9a0ee022fbe3eb6648a6a10719fc1a59750df8ac011b2c441e82539108b7da52cce47134f6129db4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1470394507;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3549237;
        ...
        assert(block.hashMerkleRoot == uint256("0xf16bcf01f31d081179ce9a751e5fb9068f34b18875d18fc5c105d5c1069a3ea1"));
...
unsigned char pchMessageStart[4] = { 0xaf, 0x0b, 0x9d, 0xb1 }; // lifecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("lifecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("lifecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static lifecoinMiner(CWallet *pwallet)
{
    printf("lifecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lifecoin-miner");
    ...
    try { loop {
        ...
        printf("Running lifecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("lifecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&lifecoinMiner, pwallet));
}
